...
- (void)testLoadRewardedAdForConfigurationWithValidApplicationKey {
    // Arrange
    id mockCredentials = OCMClassMock([GADMediationCredentials class]);
    id mockAdConfiguration = OCMClassMock([GADMediationRewardedAdConfiguration class]);
    // id mockCompletionHandler = OCMClassMock([GADMediationRewardedLoadCompletionHandler class]);
    id mockAdRequestBuilder = OCMClassMock([ISARewardedAdRequestBuilder class]);
    id mockRewardedAdLoader = OCMClassMock([ISARewardedAdLoader class]);
    id mockRewardedAdRequest = OCMClassMock([ISARewardedAdRequest class]);

    NSDictionary *credentials = @{ GADMAdapterIronSourceAppKey: @"valid_key", GADMAdapterIronSourceInstanceId: @"instance_id" };

    // Stub the `credentials` and `settings` methods
    OCMStub([mockAdConfiguration credentials]).andReturn(mockCredentials);
    NSDictionary *expectedSettings = @{ @"key1": @"value1", @"key2": @"value2" };
    OCMStub([mockCredentials settings]).andReturn(expectedSettings);
    OCMStub([mockAdConfiguration bidResponse]).andReturn(@"valid_bid_response");
    OCMStub([mockAdConfiguration watermark]).andReturn([@"valid_watermark" dataUsingEncoding:NSUTF8StringEncoding]);
    
    OCMStub([mockAdRequestBuilder alloc]).andReturn(mockAdRequestBuilder);
    OCMStub([mockAdRequestBuilder initWithInstanceId:OCMOCK_ANY adm:OCMOCK_ANY]).andReturn(mockAdRequestBuilder);
    OCMStub([mockAdRequestBuilder withExtraParams:OCMOCK_ANY]).andReturn(mockAdRequestBuilder);
    OCMStub([mockAdRequestBuilder build]).andReturn(mockRewardedAdRequest);

    OCMStub([mockRewardedAdLoader loadAdWithAdRequest:mockRewardedAdRequest delegate:OCMOCK_ANY]).andDo(nil);
    ...
    // Assert
    OCMVerify([mockRewardedAdLoader loadAdWithAdRequest:OCMOCK_ANY delegate:self.adapter]);
}


- (void)testLoadRewardedAdForConfigurationWithValidApplicationKey1 {
    // Arrange
    id mockCredentials = OCMClassMock([GADMediationCredentials class]);
    id mockAdConfiguration = OCMClassMock([GADMediationRewardedAdConfiguration class]);
    id mockAdRequestBuilderClass = OCMClassMock([ISARewardedAdRequestBuilder class]);
    id mockAdRequestBuilder = OCMClassMock([ISARewardedAdRequestBuilder class]);
    id mockRewardedAdLoaderClass = OCMClassMock([ISARewardedAdLoader class]);
    id mockRewardedAdRequest = OCMClassMock([ISARewardedAdRequest class]);
    ...
    // Stub the `credentials` and `settings` methods
    OCMStub([mockAdConfiguration credentials]).andReturn(mockCredentials);
    OCMStub([mockCredentials settings]).andReturn(credentials);
    OCMStub([mockAdConfiguration bidResponse]).andReturn(@"valid_bid_response");
    OCMStub([mockAdConfiguration watermark]).andReturn([@"valid_watermark" dataUsingEncoding:NSUTF8StringEncoding]);
    
    // Stub the request builder class methods
    OCMStub([mockAdRequestBuilderClass alloc]).andReturn(mockAdRequestBuilder);
    OCMStub([mockAdRequestBuilder initWithInstanceId:@"instance_id" adm:@"valid_bid_response"]).andReturn(mockAdRequestBuilder);
    OCMStub([mockAdRequestBuilder withExtraParams:@{GADMAdapterIronSourceWatermark: @"valid_watermark"}]).andReturn(mockAdRequestBuilder);
    OCMStub([mockAdRequestBuilder build]).andReturn(mockRewardedAdRequest);

    // Stub the class method `loadAdWithAdRequest:delegate:`
    OCMStub([mockRewardedAdLoaderClass loadAdWithAdRequest:mockRewardedAdRequest delegate:OCMOCK_ANY]);

    XCTestExpectation *completionExpectation = [self expectationWithDescription:@"Completion handler called"];
    
    // Define the completion handler block
    GADMediationRewardedLoadCompletionHandler completionHandler = ^id<GADMediationRewardedAdEventDelegate> (id<GADMediationRewardedAd> _Nullable ad, NSError * _Nullable error) {
        // Assert that the completion handler is called without errors
        XCTAssertNotNil(ad);
        XCTAssertNil(error);
        [completionExpectation fulfill];
        return nil;
    ...
    // Assert
    OCMVerify([mockRewardedAdLoaderClass loadAdWithAdRequest:mockRewardedAdRequest delegate:self.adapter]);
